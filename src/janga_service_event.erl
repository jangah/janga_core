-module(janga_service_event).

-behaviour(gen_event).

-define(NAME(), list_to_atom(atom_to_list(node()) ++ "@" ++ "service_event_manager")).
-define(NAME(Node), list_to_atom(Node ++ "@" ++ "service_event_manager")).
%% API
...
start_link(Service) ->
    gen_event:start_link({global, list_to_atom(atom_to_list(node()) ++ "@" ++ Service)}).
...
add_handler(Node) ->
    ...
    %% either goes down.
    ok = gen_event:add_handler({global, ?NAME(Node)}, ?MODULE, [self()]).
